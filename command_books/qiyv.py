...
SKILL_ROTATION_BLACKLIST = ['king','杰诺斯']
...
def step(direction, target):
    ...
    if direction in ['left', 'right']:
        ...
        key_down(direction)
        # 根据距离调整移动时间
        move_time = min(abs(d_x) * 2, 1.0)  # 最大移动时间1秒
        time.sleep(move_time)
        # 释放方向键
        key_up(direction)
        time.sleep(0.3)
    else:
        # 其他方向（向下）仍然使用TELEPORT
        press(direction, num_presses)
        time.sleep(0.8)
...
class Teleport(Command):
    """
    Moves in a given direction, jumping if specified. Adds the player's position
...
class 主攻(Command):
    ...
    def main(self):
        ...
        time.sleep(0.05)
        # 按下方向键
        press(self.direction, 1,down_time=0.03, up_time=0.03)
        # 短暂延迟
        time.sleep(0.05)
...
class 吹雪(Command):
    """Uses 吹雪 once (6 sec cd)."""

    def main(self):
        press(Key.吹雪, 2, up_time=0.05)

class 龙卷(Command):
    """Uses 龙卷 once (21 sec cd)."""

    def main(self):
        press(Key.龙卷, 2, up_time=0.05)

class 杰诺斯(Command):
    """Uses 杰诺斯 once (180 sec cd)."""

    def main(self):
        press(Key.杰诺斯, 2, up_time=0.05)

class king(Command):
    """Uses king once (90 sec cd)."""

    def main(self):
        press(Key.king, 2, up_time=0.05)
